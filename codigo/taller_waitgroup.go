...
func SimularTallerWaitGroup(taller *TallerWaitGroup, cochesA, cochesB, cochesC int) {
	totalCoches := cochesA + cochesB + cochesC
	taller.wg.Add(totalCoches)

	coches := generarCoches(cochesA, cochesB, cochesC)

	fmt.Printf("\n🏁 INICIANDO SIMULACIÓN WAITGROUP con %d coches\n", totalCoches)
	...
	fmt.Printf("\n🎉 TODOS LOS %d COCHES TERMINADOS (WaitGroup con Prioridad)\n", totalCoches)
...
func (t *TallerWaitGroup) logEvento(coche *Coche, fase int, estado string) {
	// Emoji de prioridad
	emojiPrioridad := "🟡"
